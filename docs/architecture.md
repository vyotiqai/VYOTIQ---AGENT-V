...
**Deferred gaps:** MCP resources/prompts, agent-callable `ask_question` / `switch_mode`.
...
**Diagnostics:** `diagnostics` runs project-wide typecheck (default) or lint. `read_lints` takes workspace paths and returns file-scoped eslint (when available) plus typecheck diagnostics filtered to those paths. Optional Settings → Agent → **Diagnostics command** overrides typecheck; otherwise package scripts (`typecheck` / `type-check` / `lint`) or `tsc`/`eslint` are used.
