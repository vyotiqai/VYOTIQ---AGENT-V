...
**Interaction modes (Ask / Plan / Agent):** Composer mode picker (also `/ask`, `/plan`, `/agent`). Ask exposes read-only built-ins only (including `browser_navigate` / `browser_snapshot`, not click/type); **MCP tools are not available in Ask or Plan** — `readOnlyHint` is collected but unused for mode filtering and is never trusted for parallel or approval exemption. Plan adds `todo_write` and `edit`/`str_replace` only for run artifacts `plan.md` / `contract.md`; Agent is full tools (built-ins + MCP). Mode is passed on `chatStart` and enforced by filtering tool defs plus a hard execute gate (`modePolicy.ts`). A short mode section is injected into the system prompt for Ask/Plan. After Plan drafts `plan.md`, the composer shows **Continue in Agent** (switches mode and prefills an implement prompt).
