...
describe('classifyMcpInput', () => {
  it('classifies GitHub URLs as git', () => {
    expect(classifyMcpInput('https://github.com/tirth8205/code-review-graph')).toBe('git')
  })
  ...
  it('classifies stdio commands', () => {
    expect(classifyMcpInput('uvx code-review-graph serve')).toBe('stdio')
  ...
  it('classifies npm package names', () => {
    ...
    expect(classifyMcpInput('code-review-graph')).toBe('npm')
  })
...
describe('detectFromGitRepo', () => {
  ...
  it('detects from README uvx serve line', () => {
    ...
    expect(result.server.args).toEqual(['code-review-graph', 'serve'])
...
describe('importExternalMcpServers name preservation', () => {
  ...
  it('imports servers payload with display names intact', async () => {
    ...
    const result = importExternalMcpServers({
      mode: 'merge',
      servers: [
        {
          id: 'memory-server',
