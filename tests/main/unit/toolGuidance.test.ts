...
import { TERMINAL_MAX_OUTPUT, TERMINAL_MAX_TIMEOUT_MS } from '@main/agent/tools/terminal'
...
const SECTIONS = ['LIMITS:'] as const

describe('toolGuidance', () => {
  it('covers every registered and executable built-in tool with structured sections', () => {
    ...
    for (const tool of AGENT_TOOLS) {
      ...
      expect(tool.description, `${tool.name} missing RESULT guidance`).toContain('RESULT:')
      expect(tool.description, `${tool.name} missing execution policy`).toContain('EXECUTION POLICY:')
    }
  })
  ...
  it('embeds implementation caps in LIMITS text', () => {
    ...
    expect(TOOL_GUIDANCE.terminal).toContain(String(TERMINAL_MAX_TIMEOUT_MS))
  ...
  it('states real behavioral caveats and execution policy without overclaims', () => {
    ...
    expect(TOOL_GUIDANCE.read).toMatch(/parallel.*4 concurrent/i)
    expect(TOOL_GUIDANCE.read).toMatch(/approval-exempt/i)
    expect(TOOL_GUIDANCE.web_fetch).toMatch(/parallel.*4 concurrent/i)
    expect(TOOL_GUIDANCE.web_fetch).toMatch(/Approval may be required/i)
    expect(TOOL_GUIDANCE.edit).toMatch(/Runs serially/i)
    expect(TOOL_GUIDANCE.subagent).toMatch(/2 concurrent/i)
  })
