...
`
  ...
  it('rejects empty search query (would otherwise match everything)', () => {
    const dir = mkdtempSync(join(tmpdir(), 'vyotiq-search-empty-'))
    writeFileSync(join(dir, 'a.ts'), 'x\n', 'utf8')
    expect(() => toolSearch(dir, '  ', 40)).toThrow(/required/i)
  })

  it('search is substring not regex', () => {
    ...
    const hits = toolSearch(dir, 'foo', 40)
    expect(hits).toMatch(/a\.ts/)
    expect(hits).not.toMatch(/b\.ts/)

    const literal = toolSearch(dir, 'foo.*', 40)
