...
describe('settings + composer', () => {
  ...
  it('disables harness open without workspace', () => {
    render(
      <SettingsView
        settings={{ ...baseSettings, workspacePath: null }}
        secrets={emptySecrets}
        onClose={vi.fn()}
        onUpdate={vi.fn(async () => ({ ok: true as const }))}
        onSaveSecret={vi.fn(async () => ({ ok: true as const }))}
    ...
    expect((screen.getByRole('button', { name: /Open/i }) as HTMLButtonElement).disabled).toBe(
      true
    )
  })

  it('settings show timestamps toggle persists via onUpdate', async () => {
    const onUpdate = vi.fn(async () => ({ ok: true as const }))
    render(
      <SettingsView
        settings={{ ...baseSettings, showTimestamps: false }}
        secrets={emptySecrets}
        onClose={vi.fn()}
        onUpdate={onUpdate}
        onSaveSecret={vi.fn(async () => ({ ok: true as const }))}
    ...
    const toggle = screen.getByRole('checkbox', { name: /show timestamps/i })
  ...
  it('composer uses custom model menu not select', () => {
  ...
  it('filters seed model menu for vision when images are attached', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText(/img 1/i)).toBeTruthy()
    })
