...
export function detectFromGitRepo(root: string): GitDetectResult {
  ...
  if (fromReadme) {
    if (/code-review-graph/i.test(fromReadme.name) || /code-review-graph/i.test(root)) {
      warnings.push(
        'After connecting, run `code-review-graph build` once in your workspace so graph tools have data.'
      )
    }
