...
export type HeldOutCase = {
  ...
  description: string
  receipts: RunReceipt[]
  subagentReports?: ParsedSubagentReport[]
  expect: {
...
export const HELD_OUT_CASES: readonly HeldOutCase[] = [
  ...
  {
    id: 'recurring-subagent-task',
    description: 'Repeated sub-agent tasks map to system_prompt; failed report to tool_policy',
    receipts: [
      baseReceipt({
        runId: 'held-out-subagent',
        toolStats: { totalCalls: 2, ok: 2, failed: 0, byName: { subagent: { ok: 2, failed: 0 } } }
      })
    ],
    subagentReports: [
      {
        ...
        report: 'could not determine location'
      },
      {
        id: 'sa-2',
        ok: true,
...
export function gradeHeldOutCase(heldOut: HeldOutCase): HeldOutCaseGrade {
  ...
  const summary = summarizeWeaknesses(collected, heldOut.subagentReports ?? [])
