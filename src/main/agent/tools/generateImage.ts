...
function defaultRelativePath(prompt: string, mimeType?: string): string {
  ...
  return posix.join(GENERATED_IMAGE_DIR, `${slugPrompt(prompt)}-${stamp}${ext}`)
}
...
function writeImageFile(
  ...
  try {
    ...
    return { ok: false, error: `Failed to write image inside workspace: ${message}` }
  }
}
...
export async function toolGenerateImage(
  ...
  for (let i = 0; i < extraRels.length; i++) {
    ...
    if (!w.ok) {
      return { ok: false, summary: 'error', content: w.error }
    }
  }
