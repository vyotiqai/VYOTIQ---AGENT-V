...
import { scrubValue } from '../../../shared/scrub'
...
export async function initRendererLogging(): Promise<void> {
  ...
  const backend: LoggerBackend = {
    log: (level, message, fields) => {
      ...
      const { scope: _s, correlationId: _c, err: _e, ...rest } = fields ?? {}
      const meta = Object.keys(rest).length ? (scrubValue(rest) as LogFields) : undefined
