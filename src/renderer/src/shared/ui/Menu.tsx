  ...
  const dropdown =
    open && position ? (
      <div
        ...
        style={{
          top: placement === 'up' ? undefined : position.top,
          bottom: placement === 'up' ? window.innerHeight - position.top : undefined,
          left: undefined,
          ...
          >
            {filtered.length === 0 ? (
              ...
              [...groups.entries()].map(([group, opts]) => (
                <li key={group ?? '__default'} role="presentation">
                  {group ? (
                    <div className="px-2 pt-1 pb-0.5 text-xs tracking-[var(--vy-tracking)] text-muted">
                      {group}
                    </div>
                  ) : null}
                  <ul className="m-0 list-none p-0" role="group" aria-label={group || undefined}>
                    {opts.map((opt) => {
                      flatIndex += 1
