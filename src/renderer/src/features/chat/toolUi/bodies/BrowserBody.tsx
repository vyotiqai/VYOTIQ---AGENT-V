...
export function BrowserSnapshotBody({ tool, loading, loadFailed }: ToolBodyProps) {
  ...
  return (
    <div>
      ...
      {data.refs.length > 0 ? (
        <ul className={`${TOOL_BODY_INNER} m-0 max-h-32 list-none space-y-0.5 overflow-auto p-0`}>
          {data.refs.map((ref) => (
            <li
              key={ref.id}
      ...
      {data.body ? (
        <pre
          className={`${TOOL_BODY_INNER} m-0 max-h-32 overflow-auto font-mono text-[11px] leading-relaxed whitespace-pre-wrap text-fg/75 [overflow-wrap:anywhere]`}
        >
          {data.body}
        </pre>
      ) : null}
...
export function BrowserTabsBody({ tool, loading, loadFailed }: ToolBodyProps) {
  ...
  return (
    <div>
      ...
      {tool.contentTruncated ? <TruncatedBanner loading={loading} failed={loadFailed} /> : null}
      {data.tabs.length > 0 ? (
        <ul className={`${TOOL_BODY_INNER} m-0 max-h-48 list-none space-y-1 overflow-auto p-0`}>
          {data.tabs.map((tab) => (
