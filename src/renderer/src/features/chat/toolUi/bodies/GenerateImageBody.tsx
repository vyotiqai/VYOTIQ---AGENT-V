...
export function GenerateImageBody({
  ...
  const progressText =
    running && subagent && subagent.length > 0
      ? (subagent[subagent.length - 1]?.text ?? null)
      : null
  ...
  return (
    <div>
      ...
      {data.prompt ? (
        <div className={`${TOOL_BODY_INNER} max-h-32 overflow-auto text-[11px] text-fg/80`}>
          {data.prompt}
        </div>
      ) : data.body && !previewSrc ? (
        <pre
          className={`${TOOL_BODY_INNER} max-h-40 overflow-auto whitespace-pre-wrap font-mono text-[10px] text-fg/70`}
        >
          {data.body}
        </pre>
      ) : null}
