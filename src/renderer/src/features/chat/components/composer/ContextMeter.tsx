...
function StepUsageSection({ totals }: { totals: StepUsageTotals }) {
  if (totals.steps <= 0 && totals.outputTokens <= 0 && totals.cachedInputTokens <= 0) {
    return null
  }
  const cachePct =
...
function ContextMeterPanel({ usage }: { usage: ContextUsageState }) {
  const ratio = Math.min(1, usage.used / usage.window)
  const pct = Math.round(ratio * 100)
  const compactionPct =
    usage.window > 0 ? Math.min(100, Math.round((usage.compactionTrigger / usage.window) * 100)) : 0
  const estimateDelta =
    ...
    <div className="flex flex-col gap-3 p-3 text-left">
      <div>
        ...
        <p className="mt-0.5 text-[11px] text-muted">
          Step {usage.step} · effective content budget {formatTokens(usage.contentWindow)}
        </p>
      </div>
      ...
      <p className="text-[10px] text-muted">
        ...
        {formatPct(usage.compactionTrigger, usage.contentWindow)} of content budget)
      </p>
      ...
      <div className="flex flex-col gap-2">
        ...
        <LayerRow label="System" tokens={usage.layers.system} total={usage.window} />
        <LayerRow label="History" tokens={usage.layers.history} total={usage.window} />
        <LayerRow label="Tools" tokens={usage.layers.tools} total={usage.window} />
        <LayerRow
          label="Buffer"
          tokens={usage.layers.buffer}
          total={usage.window}
          hint="(reserved budget)"
        />
        <p className="text-[10px] text-muted">
