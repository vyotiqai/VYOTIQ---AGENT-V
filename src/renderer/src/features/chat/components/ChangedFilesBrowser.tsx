...
function FileDiffBody({
  ...
  return (
    <div ref={rootRef} className="border-t border-border/40 bg-surface-2/20 px-2 py-1.5">
...
function FileRow({
  ...
  return (
    ...
    >
      <div className="flex w-full min-w-0 items-center gap-1.5 px-3 py-1.5 text-xs hover:bg-surface/60">
        <button
          type="button"
...
export function ChangedFilesBrowser({
  ...
  return (
    <div
      className={cn(
        'flex min-h-0 flex-col overflow-hidden rounded-md border border-border/50 bg-surface',
        className
      )}
    >
      ...
      <ul className="m-0 min-h-0 flex-1 list-none overflow-auto p-0" data-diff-scroll-root>
        {sorted.map((file) => (
          <FileRow
            key={file.path}
            file={file}
