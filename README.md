...
Tools: `read` · `list_dir` · `glob` · `grep` · `search` · `edit` · `str_replace` · `multi_edit` · `delete` · `todo_write` · `web_fetch` · `web_search` · `browser_navigate` · `browser_snapshot` · `browser_scroll` · `browser_click` · `browser_type` · `browser_fill` · `browser_tabs` · `browser_back` · `browser_forward` · `browser_wait_for_selector` · `browser_wait_for_url` · `browser_press_key` · `browser_select_option` · `mcp_list_tools` · `subagent` · `terminal` · `git_status` · `git_diff` · `diagnostics` · `read_lints` · `memory_list` · `memory_read` · `memory_write` (no terminal panel).
